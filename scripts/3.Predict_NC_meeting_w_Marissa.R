...
for (i in 1:length(year.list)) {
  ...
  # 7. save raw raster
  name.raster <- paste(year.no, "Nereo_preds_NC_28Aug22.tif", sep = '_')
  ...
  name.raster.rock <- paste(year.no, "Nereo_rock_preds_28Aug22.tif", sep = '_')
...
# save
#writeRaster(preds.stack, paste(preds.dir, "stack_Nereo_NC_gam_28Aug22.tif", sep ='/'), overwrite = T)
