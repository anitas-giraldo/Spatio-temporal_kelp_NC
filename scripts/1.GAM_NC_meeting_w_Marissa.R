...
mod<-gam1
head(mod$model)
testdata <- expand.grid(log_den_NERLUE=mean(mod$model$log_den_NERLUE),
...
ggmod.year <- ggplot(aes(x=year,y=response,fill=year), data=predicts.year %>% dplyr::filter(year != '2006')) +
  ylab("Log kelp density (60m2)")+
  xlab('Year')+
  ...
  #geom_line(aes(x=year,y=log_den_NERLUE, color='red')) %>%
  theme_classic() +
  theme(axis.text.x = element_text(angle = 45, h = 1))
#Theme1+
...
ggmod.year2 <- ggplot(aes(x=year,y=response,fill=year), data=predicts.year) +
  ylab("Log kelp density (60m2)")+
  xlab('Year')+
  scale_fill_viridis(discrete =T, option = 'B')+
  geom_bar(stat = "identity")+
  geom_errorbar(aes(ymin = response-se.fit,ymax = response+se.fit),width = 0.5) +
  geom_line(aes(x=year, y= observed), group = 1, color = 'green', size = 1.5) +
