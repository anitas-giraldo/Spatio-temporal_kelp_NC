...
# what you want to include here are the variables of your model
best_model <- read.csv(paste(k.dir, "best_models.csv", sep ='/')) 
head(best_mods)
nrow(best_mods)
...
for (i in 1:no_submodels){
  ...
  # 3.1. Get submodel name -- Your submodel is based on the resolution
  bm_name <- best_mods$modname[i]
  
  # get submodel variables --
  bm_vars <- strsplit(bm_name, split = "+", fixed = T) # "urchins" "temperature" "nutrient"
  ...
  folds <- crossv_kfold(dat2, k = no_folds) # dat2 is what varies in your loop: for each resolution
  folds
  ...
  # 4.3. Add the null model  -- maybe you don't need thi
  cv_mods <- cv_mods %>% 
    mutate(train = map(train, as_tibble)) %>%
