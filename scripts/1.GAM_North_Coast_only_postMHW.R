...
### Script last updated by : Anita Giraldo on 24 March 2022
...
levels(df.nc$survey_year)

## remove pre MHW years ----
levels(df.nc$survey_year)



df.nc <- df.nc %>%
  dplyr::filter(survey_year != "2010",
                  survey_year != "2011" ) %>%
  droplevels() %>%
  glimpse() # Rows: 2,579
...
dat.gam1 <- dat.gam1 %>%
  ...
  mutate(logden_NERLUE = log(den_NERLUE + 1),
         ...
         logden_PYCHEL = log(den_PYCHEL + 1)) %>% #glimpse()
  dplyr::select(-c(den_NERLUE, den_STRPURAD, den_PYCHEL)) %>%
  drop_na() %>%
  droplevels() %>%
  glimpse() # Rows: 460

any(is.na(dat.gam1))
length(levels(dat.gam1$site_campus_unique_ID))
...
name <- 'post'
o.dir <- paste(m.dir, "For_IIO", paste("gam", name, sep = '_'), sep ='/')

names(dat.gam1)
glimpse(dat.gam1)

pred.vars <- c(# choose substrate vars
  "mean_slope" , "mean_prob_of_rock" ,  "mean_logvrm",  "depth_mean",                       
  #"min_prob_of_rock" , 
  # choose other env variables
  "npgo_mean", #"wh.95", "wh.max", 
  # choose temperature vars
...
length(pred.vars) # 20
#pred.vars <- pred.vars[-c(1:10)]


## Divide into train and test ----
inTraining <- createDataPartition(dat.gam1$logden_NERLUE, p = 0.75, list = FALSE)
train.gam1 <- dat.gam1[ inTraining,]
test.gam1  <- dat.gam1[-inTraining,]

glimpse(train.gam1) # Rows: 165
glimpse(test.gam1) # 54
...
# remove unwanted interactions --
# length(model.set$mod.formula) # 12539
# test <- model.set$mod.formula
# #which(str_detect(test, "depth_mean, wh.max, wh.95"))
...
namep <- paste(name,"var.imp.png", sep ='_')
...
gam1 <- gam(formula = logden_NERLUE ~ s(depth_mean, k = 3, bs = "cr") + 
              s(mean_logvrm, k = 3, bs = "cr") + 
              s(Mean_Monthly_Nitrate, k = 3, bs = "cr") + 
              s(MHW_Summer_Days, k = 3, bs = "cr") +
              #te(depth_mean, wh.max, k = 3, bs = c("cr", "cr")) + 
              s(site_campus_unique_ID, zone, bs = "re") + 
              s(survey_year, bs = "re"), 
...
visreg(gam1)
#visreg(gam1, "Min_Monthly_Anomaly_Summer_Nitrate", "zone", ylab = 'log_den_NERLUE')
dev.off()
...
# save plot --
pdf(file=paste(o.dir, paste(name,subname,'logden_NERLUE',"fits.pdf",sep="_"), sep ='/'))
...
mod<-gam1
head(mod$model)
testdata <- expand.grid(logden_STRPURAD=mean(mod$model$logden_STRPURAD),
                        ...
                        depth_mean=mean(mod$model$depth_mean),
                        wh.max=mean(mod$model$wh.max),
...
testdata <- test.gam1 %>%
  dplyr::select(logden_NERLUE, logden_STRPURAD, mean_prob_of_rock, Min_Monthly_Anomaly_Summer_Nitrate,
                depth_mean, depth_mean,wh.max,
                site_campus_unique_ID, zone, survey_year)
...
p <- ggplot(predicts.all, aes(x = fit, y = logden_NERLUE, col = zone)) +        
  geom_point() +
...
my.formula <- y ~ x
p <- ggplot(predicts.all, aes(x = fit, y = logden_NERLUE)) +
...
p <- ggplot(predicts.all2, aes(x = fit, y = logden_NERLUE, col = zone)) +        
  geom_point() +
  labs(x='Predicted', y='Observed', title='N. luetkeana') +
  theme_bw()
p
...
my.formula <- y ~ x
p <- ggplot(predicts.all2, aes(x = fit, y = logden_NERLUE)) +
  geom_smooth(method = "lm", se=FALSE, color="black", formula = my.formula) +
  stat_poly_eq(formula = my.formula, 
               aes(label = paste(..eq.label.., ..rr.label.., sep = "~~~")), 
               parse = TRUE) +         
