...
name <- 'V6'
o.dir <- paste(m.dir, "outputs_nc", paste("gam", name, sep = '_'), sep ='/')

names(dat.gam1)
glimpse(dat.gam1)

pred.vars <- c(# choose substrate vars
  "mean_slope" , "mean_prob_of_rock" ,  "mean_logvrm",                         
  "min_prob_of_rock" , "depth_mean",
  # choose other env variables
  "npgo_mean", "wh.95", "wh.max", 
  # choose temperature vars
  "Max_Monthly_Anomaly_Summer_Temp", 
...
inTraining <- createDataPartition(dat.gam1$logden_PYCHEL, p = 0.75, list = FALSE)
...
gam1 <- gam(formula = logden_NERLUE ~ s(logden_STRPURAD, k = 3, bs = "cr") + 
