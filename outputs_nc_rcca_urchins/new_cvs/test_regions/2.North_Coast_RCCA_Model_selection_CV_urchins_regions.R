...
# last modified by Anita Giraldo - 14 Abril 2023
...
cv.dir <- paste(o.dir, "new_cvs", "test_regions", sep ='/')
...
dat1 <- df.nc %>%
  dplyr::select(
    ...
    UBR_Mean, UBR_Max,
    # NPP
    Mean_Monthly_NPP, Max_Monthly_NPP, Max_Monthly_NPP_Upwelling, Min_Monthly_NPP_Upwelling, Mean_Monthly_NPP_Upwelling, Min_Monthly_NPP) %>%
  # Bio transformations
  ...
  mutate(log_Mean_Monthly_NPP_Upwelling = log(Mean_Monthly_NPP_Upwelling + 1),
         log_Min_Monthly_NPP_Upwelling = log(Min_Monthly_NPP_Upwelling + 1),
         log_Min_Monthly_NPP = log(Min_Monthly_NPP + 1)) %>%
  dplyr::select(-c(Mean_Monthly_NPP_Upwelling,
                   Min_Monthly_NPP,
                   Min_Monthly_NPP_Upwelling)) %>%
  glimpse() # Rows: 708
...
best_mods <- read.csv(paste(cv.dir, "best_models_regions.csv", sep ='/')) 
head(best_mods)
...
submodel_names <- c("bmnc", "bmcc", "bmsc")
